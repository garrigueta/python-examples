...
def get_sin_oscillator(freq=55, amp=1, sample_rate=SAMPLE_RATE):
    ...
    return (math.sin(v) * amp * NOTE_AMP for v in itertools.count(start=0, step=increment))


def get_samples(notes_dict, num_samples=FRAMES_PER_BUFFER):
    return [sum([int(next(osc) * 127) for _, osc in notes_dict.items()]) for _ in range(num_samples)]
